...
import { submitArrival } from "@/services/checkinService";
import { uploadDocument } from "@/services/documentService";
import { signDocument as apiSignDocument } from "@/services/signatureService";
import { getAlerts } from "@/services/alertService";
import { connectRealtime, onRealtimeEvent } from "@/services/realtimeService";
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // Real DB ID for the active load (null = using mock data)
  const apiLoadIdRef = useRef<string | null>(null);
  ...
  // Try loading real trip data from the API (non-blocking — falls back to mock)
  useEffect(() => {
    getCurrentTrip()
      .then((apiLoad) => {
        apiLoadIdRef.current = apiLoad.id;
        const mapped: Load = {
          id: apiLoad.id,
          carrier: apiLoad.carrier,
          pickupFacility: apiLoad.pickupFacility ?? "—",
          pickupAddress: apiLoad.pickupAddress ?? "—",
          deliveryFacility: apiLoad.deliveryFacility ?? "—",
          deliveryAddress: apiLoad.deliveryAddress ?? "—",
          appointmentTime: apiLoad.appointmentTime ? new Date(apiLoad.appointmentTime) : new Date(),
          truckNumber: apiLoad.truckNumber ?? "—",
          trailerNumber: apiLoad.trailerNumber ?? "—",
          driverPhone: apiLoad.driverPhone ?? "—",
          loadNumber: apiLoad.loadNumber,
          referenceNumber: apiLoad.referenceNumber ?? "—",
          poNumber: apiLoad.poNumber ?? "—",
          status: (apiLoad.status as DriverStatus) ?? "en_route",
          eta: apiLoad.eta ?? "—",
          distance: apiLoad.distance ?? "—",
          dockAssignment: apiLoad.dockAssignment,
          queuePosition: apiLoad.queuePosition,
          instructions: apiLoad.instructions,
          arrivedTime: apiLoad.arrivedTime ? new Date(apiLoad.arrivedTime) : undefined,
          checkInTime: apiLoad.checkInTime ? new Date(apiLoad.checkInTime) : undefined,
          statusHistory: [{ status: (apiLoad.status as DriverStatus) ?? "en_route", timestamp: new Date() }],
        };
        setLoads([mapped]);
        setActiveLoadId(mapped.id);
      })
      .catch(() => { /* no API — keep mock data */ });
  }, []);

  // Load real alerts from API and merge into notifications
  useEffect(() => {
    getAlerts()
      .then((apiAlerts) => {
        const mapped: AppNotification[] = apiAlerts.map((a) => ({
          id: a.id,
          type: (a.type as AppNotification["type"]) ?? "arrival",
          title: a.title,
          message: a.message,
  ...
  useEffect(() => {
    ...
    const unsubs = [
      onRealtimeEvent("dock_assigned", (payload: any) => {
        if (payload?.loadId === apiLoadIdRef.current) {
          updateActiveLoad((prev) => ({
            ...prev,
            status: "dock_assigned",
            dockAssignment: payload.dockName,
            statusHistory: [...prev.statusHistory, { status: "dock_assigned", timestamp: new Date() }],
          }));
          setDriverNotifications((prev) => [
            { id: Date.now().toString(36), type: "dock_assigned", title: "Dock Assigned", message: `Dock ${payload.dockName} assigned for Load ${payload.loadNumber}.`, time: new Date(), read: false },
            ...prev,
          ]);
        }
      }),
      onRealtimeEvent("arrival_confirmed", (payload: any) => {
        if (payload?.loadId === apiLoadIdRef.current) {
          updateActiveLoad((prev) => ({
            ...prev,
            status: "arrived",
            arrivedTime: new Date(),
            statusHistory: [...prev.statusHistory, { status: "arrived", timestamp: new Date() }],
          }));
        }
      }),
    ];
  ...
  const simulateArrival = useCallback(() => {
    ...
    const newArrival: Arrival = {
      ...
      driverPhone: currentLoad.driverPhone,
  ...
  const submitCheckIn = useCallback((data: CheckInFormData) => {
    ...
    if (apiLoadIdRef.current) {
      submitArrival({ loadId: apiLoadIdRef.current, notes: data.notes || undefined }).catch(() => {});
    }
  }, []);
  ...
  const signDocument = useCallback((docId: string, data: { signatureData: string; signatureType: "drawn" | "typed"; fieldType: SigFieldType }) => {
    ...
    // Persist to API if the docId looks like a real UUID (not a mock "doc-xxx" id)
    if (/^[0-9a-f]{8}-/.test(docId)) {
      apiSignDocument(docId, {
        signatureData: data.signatureData,
        signatureType: data.signatureType,
        fieldType: data.fieldType,
        loadNumber: currentLoad.loadNumber,
